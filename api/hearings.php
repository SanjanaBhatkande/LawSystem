    ...
    if ($method === 'GET' && $action === 'list') {
        ...
        $stmt = $pdo->prepare(
            "INSERT INTO Hearings (HearingRef, CaseID, JudgeID, HearingDate, Mode, Room, Status)
             VALUES ('', :case, :judge, :date, :mode, :room, :status)"
        );
        ...
        $ref = "H-" . date("Y") . "-" . str_pad($lastId, 5, "0", STR_PAD_LEFT);

        $pdo->prepare("UPDATE Hearings SET HearingRef=? WHERE HearingID=?")
            ->execute([$ref, $lastId]);
        ...
        $pdo->prepare("DELETE FROM Hearings WHERE HearingID = ?")
            ->execute([$id]);
